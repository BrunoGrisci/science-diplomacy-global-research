...
def main():
    ...
    counter_file = csv_final_path.replace('.csv', '_counter_results.csv')
    ...
    centrality_df.to_csv(csv_final_path.replace('.csv', '_centrality.csv'), index=False)
    ...
    pyvis_graph.show(csv_final_path.replace('.csv', '.html'), notebook=False)
