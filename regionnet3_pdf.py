...
node_size_dict = {node: in_degree_centrality.get(node, 0.01) * 1000 for node in G.nodes()}
node_color_dict = {}
for node in G.nodes:
    continent = country_to_region.get(node, None)
    if not continent:
        continent = subregion_to_region.get(node, [node, node])
    color = get_color(continent[1])
...
plot_graph_pdf(
    G,
    output_path=output_dir + csv_final_path.replace('.csv', '_regions_network.pdf'),
